...
* **Add two AI agents to expand company-level recommendation coverage:**
  ...
  * The second agent uses these attributes as constraints to identify the **top-k similar companies** for each interview company.
  ...
  The underlying rationale is that if a company with a particular industry and business profile has shown interest in my profile, companies with similar business characteristics may also represent relevant opportunities.

* **Optimize job alerts based on company-level recommendations:**
  * The system now builds a list of companies that are most similar to my historical interview companies and uses them as **priority job-alert companies**.
  * Newly posted roles from these companies are automatically assigned **Priority 1**, allowing them to be reviewed and applied to first.

* **Optimize job recommendation priorities:**
  * **Priority 1:** Newly opened roles from the AI agent-recommended company list described as above.
  * **Priority 2:** Roles generated by the existing customized recommendation engine, excluding any `job_id` already included in Priority 1 to avoid duplicate review.
  * **Priority 3:** All other newly discovered jobs, excluding `job_id`s already included in Priorities 1 and 2. These roles can be considered when time permits and can be safely ignored when application capacity is limited.

* **Add application-context features:**
  * The recommendation system now incorporates the **most recent application title, application date, and number of days since the latest application**.
  * This provides additional context when deciding whether to apply to a newly opened role. For example, if I applied for a Senior Data Scientist position yesterday but a Senior AI Engineer position opens today, the system can still identify the new role as a potentially valuable application despite the short interval.

* **Automate historical application data maintenance:**
  * As long as the system continues to be used, historical application data is maintained automatically.
  * This eliminates the need to manually retrieve and incorporate historical application records when generating new recommendations.
